...
class _AboutPage(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        _changelog_entries = [
            ("0.3.13.3","e32b4ef", "start button after crash, delete dialog buttons, buffer overshoot, compress retry", False),
            ("0.3.13.2","24bd92a", "replace single-instance lock with TCP, add startup log", True),
            ("0.3.13.1","802bcd7", "fix psutil import crash on restart", True),
            ("0.3.13",  "32f2f33", "keyboard trigger — pynput GlobalHotKeys, detect dialog, settings UI", False),
            ("0.3.12.1","a43400a", "skip compression if clip already under target size", True),
            ("0.3.12",  "0955a38", "Windows capture: DXGI ddagrab with gdigrab fallback", False),
            ("0.3.11.1","01c6060", "monitor dropdown populated on Linux and Windows", True),
            ("0.3.11",  "d450771", "Windows auto-monitor captures primary, skip NVENC if resolution too high", False),
            ("0.3.10.9","17ceb8d", "mode change triggers recorder restart, trigger button reloads live", True),
            ("0.3.10.8","19e9256", "fix Windows fontconfig crash in drawtext, /dev/null→NUL in ffmpeg", True),
            ("0.3.10.5","235c388", "WASAPI no-loopback fallback before dropping audio entirely", True),
            ("0.3.10.4","c66624d", "register Windows AUMID so toast notifications actually show", True),
            ("0.3.10.3","91be9b7", "drain ffmpeg stderr continuously to prevent gdigrab RAM explosion", True),
            ("0.3.10",  "257265c", "fall back to no-audio recording if WASAPI loopback unsupported", False),
            ("0.3.9.9", "e023985", "fix mic controls greyed out on init when no mic selected", True),
            ("0.3.9.8", "5b4463d", "log ffmpeg stderr on crash, fix empty WASAPI device string", True),
            ("0.3.9.7", "759990d", "number duplicate WASAPI device names in audio dropdowns", True),
            ("0.3.9.6", "1d4e159", "ffmpeg startup check with winget auto-install, Windows update checker", False),
            ("0.3.9.5", "fa5643c", "recorder restart on settings change, confirm dialog for buffer saves", False),
            ("0.3.9.3", "80aa2d1", "fix winreg CPU/audio enum, WASAPI loopback, os.startfile clips", True),
            ("0.3.9.2", "361ab8d", "ffmpeg dep check on Windows, add quit button, fix restart DLL error", True),
            ("0.3.9",   "5e676f3", "replace OBS with ffmpeg gdigrab recorder, fix daemon startup", False),
            ("0.3.8",   "98b1e6a", "MAUVE color, CPU/RAM detection, log folder, notifications, file actions", False),
            ("0.3.7.9", "5cc76d9", "QMessageBox button styles for Windows, fix MittenConfig default init", True),
            ("0.3.7.8", "409a2ec", "Windows daemon start/stop via sys.executable + psutil", True),
            ("0.3.7",   "8498576", "Windows 10 port — OBS recorder, pynput, CI exe build", False),
